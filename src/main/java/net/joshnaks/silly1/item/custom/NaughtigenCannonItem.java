...
public class NaughtigenCannonItem extends BowItem
{
    private boolean charged = false;

    public NaughtigenCannonItem(Settings settings) {
        super(settings);
    }
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (f >= 1.0F && !isCharged(stack))
        {
            if(!world.isClient)
            {
                ...
                NaughtigenProjectile projectileEntity = new NaughtigenProjectile(world, user);
